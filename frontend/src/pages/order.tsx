...
export default function Order() {
  ...
  const applyDirectPaymentResult = async (payment: DirectPaymentRecord) => {
    ...
    if (payment.status === 'success' && payment.order) {
      const providerSuccess = payment.order.order_status === 'success';
      setResult(providerSuccess ? 'Provider berhasil. Akun siap dilihat.' : 'Pembayaran berhasil. Provider sedang memproses pesanan.');
      setLastOrder({
      ...
      setResult('Pembayaran berhasil. QRIS ditutup dan pesanan sedang diproses.');
      window.dispatchEvent(new Event('premiuminplus:balance-updated'));
    }
  };
  ...
  const startDirectPayment = async () => {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : 'Gagal membuat QRIS pembayaran.');
    } finally {
  ...
  const cancelDirectPayment = async () => {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : 'Gagal membatalkan pembayaran.');
    } finally {
      setDirectLoading(false);
    }
  };
          ...
          <div className="flex h-full flex-col justify-between gap-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-2xl border border-white/10 bg-black/20 px-4 py-3">
                ...
                <p className="mt-1 text-[11px] text-white/40">Usable {formatCurrency(usableBalance)}</p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-black/20 px-4 py-3">
              ...
              <div className="rounded-2xl border border-emerald-500/20 bg-[#0b0f1a] p-4">
                ...
                <div className="mt-3 grid gap-2 text-sm text-white/70 sm:grid-cols-2">
                  {lastOrder.status === 'success' ? (
                    ...
                    <span className="sm:col-span-2">Provider masih memproses pesanan. Akun akan tampil setelah status success.</span>
                  )}
                </div>
              </div>
            ) : null}
              ...
              <div className="space-y-3">
                ...
                {directPayment.status === 'success' && directPayment.order?.order_status === 'success' ? (
                  <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-100">
                    <p className="font-black">Pesanan berhasil</p>
                  ...
                  <p className="text-sm leading-6 text-white/55">Pembayaran berhasil. Provider sedang memproses pesanan; akun belum ditampilkan sampai provider success.</p>
                ) : (
                ...
                <div className="grid gap-2">
                  ...
                  {directPayment.status === 'success' ? (
                    <button onClick={() => setDirectPayment(null)} className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-bold text-white/80">
                      Tutup
                    </button>
                  ) : (
