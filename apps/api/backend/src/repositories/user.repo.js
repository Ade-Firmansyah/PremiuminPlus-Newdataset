...
function toPublicUser(row) {
  ...
  return {
    ...
    usable_balance: Math.max(Number(row.saldo || 0) - Number(row.locked_balance || 0), 0),
    bot_access_unlocked: Boolean(row.bot_access_unlocked),
    bot_disabled_reason: row.bot_disabled_reason || '',
    reseller_request_status: row.reseller_request_status || 'none',
...
export async function findUserByUsername(username) {
  const rows = await query('SELECT * FROM users WHERE LOWER(username) = LOWER(?) LIMIT 1', [String(username || '').trim()]);
  return toAuthUser(rows[0] || null);
}

export async function findUserByLogin(login) {
  const value = String(login || '').trim();
  const emailValue = value.includes('@') ? value.toLowerCase() : '';
  let phoneValue = '';
  try {
    phoneValue = normalizePhoneNumber(value);
  } catch {
    phoneValue = '';
  }
  const rows = await query(
    `SELECT * FROM users
     WHERE LOWER(username) = LOWER(?)
        ...
        OR phone = ?
     LIMIT 1`,
    [value, emailValue || value, phoneValue || '__no_phone_match__'],
  );
  return toAuthUser(rows[0] || null);
}

export async function findUserByEmail(email) {
  const rows = await query('SELECT * FROM users WHERE LOWER(email) = LOWER(?) LIMIT 1', [String(email || '').trim()]);
  return toAuthUser(rows[0] || null);
}
...
export async function findUserForPasswordReset({ username, email, phone, identifier }) {
  if (identifier !== undefined) return findUserByLogin(identifier);
  const rows = await query(
    `SELECT * FROM users
     WHERE LOWER(username) = LOWER(?) AND LOWER(email) = LOWER(?) AND phone = ?
     LIMIT 1`,
...
export async function regenerateUserApiKey(id) {
  const userId = toSafeInteger(id, 'ID user');
  const current = await getUserById(userId);
  if (!current) return null;
  const apiKey = generateApiKey(current.username);
  await execute('UPDATE users SET api_key = ? WHERE id = ?', [apiKey, userId]);
...
export async function touchUserLastLogin(userId) {
  const safeUserId = toSafeInteger(userId, 'ID user');
  await execute('UPDATE users SET last_login_at = NOW() WHERE id = ?', [safeUserId]);
}
...
function userInsertSql() {
  return `INSERT INTO users
      (username, email, phone, password_hash, password, api_key, saldo, locked_balance, bot_access_unlocked, bot_disabled_reason, reseller_request_status, markup_custom, markup_percent, reseller_margin_percent, theme, fullName, orders, deposits, notes, role, status, last_login_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
}

function userInsertParams(payload, password_hash, api_key) {
  return [
    payload.username,
    payload.email || null,
    payload.phone || null,
    ...
    api_key,
    0,
    Number(payload.locked_balance || 0),
    payload.bot_access_unlocked ? 1 : 0,
    payload.bot_disabled_reason || null,
    payload.reseller_request_status || 'none',
...
export async function createUser(payload) {
  ...
  const result = await execute(
    userInsertSql(),
    userInsertParams(payload, password_hash, api_key),
  );
...
export async function createUserAtomic(payload, { activity = null } = {}) {
  const passwordValue = String(payload.password || '');
  const password_hash = isHashedPassword(passwordValue) ? passwordValue : hashPassword(passwordValue);
  const api_key = payload.api_key || generateApiKey(payload.username);

  return transaction(async (connection) => {
    const [result] = await connection.query(userInsertSql(), userInsertParams(payload, password_hash, api_key));
    const userId = toSafeInteger(result.insertId, 'ID user');
    if (activity) {
      try {
        await connection.query(
          `INSERT INTO activity_logs (actor_id, user_id, scope, message, activity, action, ip_address, metadata)
           VALUES (?, ?, ?, ?, ?, ?, ?, CAST(? AS JSON))`,
          [
            userId,
            userId,
        ...
        // Activity log is intentionally best-effort; registration must stay atomic for user data.
      }
    }
    const [rows] = await connection.query('SELECT * FROM users WHERE id = ? LIMIT 1', [userId]);
    return toPublicUser({ ...(rows[0] || {}), api_key });
  });
}

export async function updateUser(id, payload) {
  const userId = toSafeInteger(id, 'ID user');
  const currentRows = await query('SELECT * FROM users WHERE id = ? LIMIT 1', [userId]);
  ...
  const nextData = {
    ...
    locked_balance: payload.locked_balance !== undefined ? Number(payload.locked_balance) : current.locked_balance,
    bot_access_unlocked: payload.bot_access_unlocked !== undefined ? (payload.bot_access_unlocked ? 1 : 0) : current.bot_access_unlocked,
    bot_disabled_reason: payload.bot_disabled_reason !== undefined ? payload.bot_disabled_reason || null : current.bot_disabled_reason,
    reseller_request_status: payload.reseller_request_status !== undefined ? payload.reseller_request_status || 'none' : current.reseller_request_status,
  ...
  await execute(
    `UPDATE users
     SET username = ?, email = ?, phone = ?, password_hash = ?, password = ?, api_key = ?, role = ?, status = ?, fullName = ?, notes = ?, last_login_at = ?, markup_custom = ?, markup_percent = ?, reseller_margin_percent = ?, locked_balance = ?, bot_access_unlocked = ?, bot_disabled_reason = ?, reseller_request_status = ?, theme = ?, orders = ?, deposits = ?
     WHERE id = ?`,
    [
      nextData.username,
      nextData.email,
      ...
      nextData.locked_balance,
      nextData.bot_access_unlocked,
      nextData.bot_disabled_reason,
      nextData.reseller_request_status,
      nextData.theme,
      nextData.orders,
      nextData.deposits,
      userId,
  ...
  const counters = await seedUserCounters(userId);
