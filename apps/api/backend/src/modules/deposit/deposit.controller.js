import { cancelDeposit, createBotActivationDeposit, createDeposit, refreshDepositStatus } from './deposit.service.js';
...
export async function botActivationDeposit(req, res, next) {
  try {
    const data = await createBotActivationDeposit(req.user);
    res.status(201).json({ status: true, success: true, data });
  } catch (error) {
    next(error);
  }
}
