  ...
  schedulePaymentWatch(session, remoteJid, invoice) {
    ...
    const statusTimer = setInterval(async () => {
      try {
        ...
        // Status polling is best-effort; web-core remains the source of truth.
      }
    }, 10 * 1000);
    ...
    const reminderTimer = setTimeout(async () => {
      try {
        ...
        // Reminder is best-effort; payment status remains owned by web-core.
      }
    }, 3 * 60 * 1000);
    ...
    const cancelTimer = setTimeout(async () => {
      try {
        ...
        // Expiry fallback is best-effort. Manual cek invoice still works.
      }
    }, 5 * 60 * 1000);
