...
function toPublicUser(row) {
  ...
  return {
    ...
    markup_percent: Number(row.markup_percent ?? row.markup_custom ?? 0),
...
export async function createUser(payload) {
  ...
  const result = await execute(
    `INSERT INTO users
      (username, email, phone, password_hash, password, api_key, saldo_utama, saldo, locked_balance, markup_custom, markup_percent, theme, fullName, orders, deposits, notes, role, status, last_login_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      payload.username,
      payload.email || null,
...
export async function updateUser(id, payload) {
  ...
  const nextData = {
    ...
    bot_enabled: payload.bot_enabled !== undefined ? (payload.bot_enabled ? 1 : 0) : current.bot_enabled,
    bot_role: payload.bot_role !== undefined ? String(payload.bot_role || 'personal').slice(0, 40) : current.bot_role,
    bot_session_status: payload.bot_session_status !== undefined ? String(payload.bot_session_status || 'disconnected') : current.bot_session_status,
    token_version: nextPassword !== current.password_hash ? Number(current.token_version || 1) + 1 : Number(current.token_version || 1),
  };

  await execute(
    `UPDATE users
     SET username = ?, email = ?, phone = ?, password_hash = ?, password = ?, api_key = ?, role = ?, status = ?, fullName = ?, notes = ?, last_login_at = ?, markup_custom = ?, markup_percent = ?, theme = ?, orders = ?, deposits = ?, locked_balance = ?, bot_enabled = ?, bot_role = ?, bot_session_status = ?, token_version = ?
     WHERE id = ?`,
    [
      nextData.username,
      nextData.email,
      ...
      nextData.bot_enabled,
      nextData.bot_role,
      nextData.bot_session_status,
      nextData.token_version,
      Number(id),
    ],
  );
